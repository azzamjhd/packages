...
#let advisors-placement(title, author, student-id, advisors, study-program, department, institution, year, colors, document-type, invert) = {
    align(right)[#text(weight: "bold", size: 20pt, fill: if invert { black } else { colors.primary })[#upper(document-type)]]
    v(1em*6)
    text(weight: "bold", size: 18pt, fill: if invert { black } else { white })[#upper(title)]
    v(13mm)
    text(weight: "bold", size: 16pt, fill: if invert { black } else { white })[#author \ #student-id]
    v(4.5mm)
    text(weight: "bold", size: 16pt, fill: if invert { black } else { white })[
      DOSEN PEMBIMBING       #for a in advisors [\ #a.name \ #a.id \ ]
    ]
...
#let cover-page(
  ...
  align(center)[
    #advisors-placement(title, author, student-id, advisors, study-program, department, institution, year, colors, document-type, false)
  ]
}
...
#let inner-cover-page(
  ...
  align(center)[
    #advisors-placement(title, author, student-id, advisors, study-program, department, institution, year, colors, document-type, true)
  ]
}
